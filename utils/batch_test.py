...
def test(model, users_to_test, drop_flag=False, batch_test_flag=False,tp=0,pc_model=None,alphas=[]):
    # print("print test function para")
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
    ...
    # print("i_batch_size:",i_batch_size)
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            ...
            for i_batch_id in range(n_item_batchs):
                ...
                if tp==0:
                    ...
                    # pc_model = PeroidCluster(k=30, dimension=64, tempture=0.5)
                    # pc_model.load_state_dict(torch.load(pc_model_path))
                    if drop_flag == False:
                        u_g_embeddings, pos_i_g_embeddings, _ =  model(interactions_test, interactions_test_time,pc_model,user_batch,item_batch,[],
                                                                       alpha1 = alphas[0],alpha2 = alphas[1],alpha3 = alphas[2])
            ...
            if tp==0:
                ...
                # pc_model = PeroidCluster(k=20, dimension=64, tempture=0.5,device="cuda:0")
                # pc_model.load_state_dict(torch.load(pc_model_path))
                if drop_flag == False:
                    u_g_embeddings, pos_i_g_embeddings, _ =  model(interactions_test, interactions_test_time,pc_model,user_batch,item_batch,[],
                                                                   alpha1 = alphas[0],alpha2 = alphas[1],alpha3 = alphas[2]
        ...
        for re in batch_result:
            result['precision'] += re['precision'] / i_batch_size
            result['recall'] += re['recall']/ i_batch_size
            result['ndcg'] += re['ndcg']/ i_batch_size
            result['hit_ratio'] += re['hit_ratio']/ i_batch_size
            result['auc'] += re['auc']/ i_batch_size
    ...
    # result['ndcg'] = result['ndcg'] 
    # result['hit_ratio'] = result['hit_ratio'] 
    # result['auc'] = result['auc'] 
    # print("result1:",result)
    #######################
    return result

def test_prompt(model, users_to_test, drop_flag=False, batch_test_flag=False):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
